...
LastEditTime: 2023-05-30 13:42:15
FilePath: /chengdongzhou/action/tRexHAR/utils/augmentations.py
...
def get_aug_data( data , RA = None , FA = None):
    if args.RandAug:
        aug_data = RA( data.squeeze(1) ).unsqueeze(1)
        inputs   = torch.cat( [data,aug_data] , dim=0 )
    elif args.FixAug:
        aug_data = FA( data.squeeze(1) , args.data_aug, args.data_aug2 ).unsqueeze(1)
        inputs   = torch.cat( [data,aug_data] , dim=0 )
    else:
